...
<body>
	...
	<h3 style="margin-bottom: 20px;"><a style="color: black; text-decoration: none;" href='<c:url value="/board/record?bc_code=${pageMaker.cri.bc_code}"/>'>${recordList[0].bc_name} 게시판</a></h3>
		...
		<div class="postTypeBtnWrapper">
			<button class="btn btn-primary" type="button" onclick="total('${title}');">전체글</button>
			<button class="btn btn-secondary" type="button" onclick="recommend('${title}');">인기글</button>
		</div>
		<div class="writeBtnWrapper">
			<button class="btn btn-light" type="button" onclick="recordRegister('${title}');">글쓰기</button>
		</div>
						...
						<c:otherwise>
								...
								<c:set var="day_comparison">
									...
									<tr>
										...
										<td><a class="title_a" onclick="goRecordDetail('${title}', '${recordList.b_num}');">${recordList.title}</a></td>
										<c:choose>
